...
import { assessProjectHealth } from '../engine/rules';
...
export function getMockERPData() {
...
export async function syncERPData(): Promise<ERPSyncResult> {
  ...
  try {
    ...
    if (useRealERP) {
      ...
      const projectUrl = `${baseUrl}/api/resource/Project?fields=["name","project_name","status","percent_complete","priority","owner","expected_start_date","expected_end_date","creation","project_type","is_active","percent_complete_method","department"]&limit_page_length=1000`;
      ...
      for (const p of rawProjects) {
        const pTasks = rawTasks.filter((t: any) => t.project === p.name);
        ...
        if (!startDate) {
          const taskStarts = pTasks
            .map((t: any) => t.exp_start_date ? new Date(t.exp_start_date).getTime() : null)
        ...
        const mappedTasks = pTasks.map((t: any) => {
          ...
          return {
            ...
            startDate: tStart as Date,
            endDate: tEnd as Date,
        ...
        erpProjects.push({
          ...
          isActive: p.is_active || "No",
          percentCompleteMethod: p.percent_complete_method || "Task Completion",
    ...
    for (const project of erpProjects) {
      ...
      const healthResult = assessProjectHealth(
      ...
      const upsertedProject = await prisma.project.upsert({
        ...
        update: {
          ...
          percentCompleteMethod: project.percentCompleteMethod,
      ...
      const aiResult = await analyzeProjectHealth(
        upsertedProject,
        project.tasks,
    ...
    return {
      ...
      details: [`Sync execution failed: ${(error as Error).message}`]
    };
  }
}
